...
export default function App() {
  ...
  // [OK] FIX PÓSTER: Cuando cambian los juegos, auto-seleccionar el primer domingo
  // que tenga partidos, para que el póster no quede siempre vacío.
  useEffect(() => {
  ...
  const recordScore = () => {
    ...
    // [OK] FIX MARCADOR: Si el partido ya fue completado, no procesar de nuevo
    // para evitar que las estadísticas de los equipos se dupliquen.
      ...
      <div className="body">
            ...
            {filtered.length===0
              ...
              : <table className="tbl"><thead><tr><th>#</th><th>Equipo</th><th>PJ</th><th>G</th><th>P</th><th>+/-</th><th>PTS</th></tr></thead>
                <tbody>{filtered.map((t,i)=><tr key={t.id}>
                  <td><span className={`rank ${i===0?"r1":i===1?"r2":i===2?"r3":""}`}>{i===0?"1":i===1?"2":i===2?"3":i+1}</span></td>
        ...
        {tab==="poster" && <>
          ...
          {gamesOn(posterSun).length===0
            ? <div className="empty"><div className="empty-i">[P]</div><div className="empty-t">Agrega partidos al Calendario para generar el póster</div></div>
        ...
        {tab==="teams" && <>
          ...
          <div className="card">
            ...
            {teams.length===0
              ? <div className="empty"><div className="empty-i">[G]</div><div className="empty-t">Sin equipos</div></div>
