...
.hdr::after{content:'🏀';position:absolute;font-size:140px;opacity:.03;right:-10px;top:-30px;}
...
export default function App() {
  ...
  const resetAll = () => {
    ...
    setTeams(INIT_TEAMS);setGames([]);setVenue("Salón Seño Zulema");setTourney("TORNEO RELÁMPAGO 2025");
    localStorage.removeItem(LS_KEY);
  };

  const TABS=[{id:"standings",label:"📊 Tabla"},{id:"schedule",label:"📅 Calendario"},{id:"scores",label:"⚡ Marcador"},{id:"poster",label:"🎨 Póster"},{id:"teams",label:"⚙️ Equipos"}];
      ...
      <div className="body">
          ...
          <div className="card">
            ...
            {gamesOn(selSun).length<4 && <>
              ...
              <div className="g2">
                ...
                <div className="fr"><label className="lbl">Categoría</label><select className="sel" value={newGame.category} onChange={e=>setNewGame({...newGame,category:e.target.value})}><option>Masculino</option><option>Femenino</option></select></div>
              </div>
            ...
            {gamesOn(selSun).length>=4 && <div className="warn" style={{marginTop:"10px"}}>⚠️ Límite de 4 partidos alcanzado</div>}
          </div>
        </>}
        ...
        {tab==="scores" && <>
          <div className="card">
            ...
            {pending.length===0
              ? <div className="empty"><div className="empty-i">🎯</div><div className="empty-t">Sin partidos pendientes.</div></div>
        ...
        {tab==="poster" && <>
          ...
          {gamesOn(posterSun).length===0
            ? <div className="empty"><div className="empty-i">🎨</div><div className="empty-t">Agrega partidos al Calendario para generar el póster</div></div>
            : <div className="poster"><div className="poster-hdr"><div className="poster-title">🏀 {tourney}</div><div className="poster-sub">Programa de Partidos — Domingo</div></div>
              <div className="poster-info"><div className="poster-info-row"><span>📅</span><span className="poster-info-lbl">Fecha:</span><span className="poster-info-val">{formatDateES(sundays[posterSun])}</span></div><div className="poster-info-row"><span>📍</span><span className="poster-info-lbl">Lugar:</span><span className="poster-info-val">{venue}</span></div></div>
        ...
        {tab==="teams" && <>
          <div className="card">
            ...
            <div className="notice">💾 Los cambios se guardan automáticamente</div>
          ...
          <div className="card">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"9px",alignItems:"end"}}>
              <div><div className="fr"><label className="lbl">Nombre del Equipo</label><input className="inp" value={newTeam.name} onChange={e=>setNewTeam({...newTeam,name:e.target.value})} placeholder="Ej: Los Águilas"/></div><div className="fr" style={{marginBottom:0}}><label className="lbl">Categoría</label><select className="sel" value={newTeam.category} onChange={e=>setNewTeam({...newTeam,category:e.target.value})}><option>Masculino</option><option>Femenino</option></select></div></div>
